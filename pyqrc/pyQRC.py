            ...
            if format == "Gaussian":
                ...
                try:
                    ...
                    for i in range(standor + 5, standor + 5 + self.NATOMS):
                        ...
                        self.CARTESIANS.append([float(outlines[i].split()[3]), float(outlines[i].split()[4]),
...
class gen_qrc:
    def __init__(self, file, amplitude, nproc, mem, route, verbose, suffix, val, num):
        ...
        if verbose:
            log = Logger(file.split(".")[0], "qrc", suffix)
        ...
        if verbose:
            ...
            log.Writeonlyfile(
                'Based on: Goodman, J. M.; Silva, M. A. Tet. Lett. 2003, 44, 8233-8236; Tet. Lett. 2005, 46, '
                '2067-2069.\n')
            log.Writeonlyfile('                -----ORIGINAL GEOMETRY------')
            log.Writeonlyfile('{0:>4} {1:>9} {2:>9} {3:>9} {4:>9}'.format('', '', 'X', 'Y', 'Z'))
            for atom in range(0, freq.NATOMS):
                log.Writeonlyfile('{0:>4} {1:>9} {2:9.6f} {3:9.6f} {4:9.6f}'.format(freq.ATOMTYPES[atom], '',
                                                                                    freq.CARTESIANS[atom][0],
                                                                                    freq.CARTESIANS[atom][1],
                                                                                    freq.CARTESIANS[atom][2]))
            log.Writeonlyfile('\n                ----HARMONIC FREQUENCIES----')
            log.Writeonlyfile('{0:>24} {1:>9} {2:>9}'.format('Freq', 'Red mass', 'F const'))
            for mode in range(0, 3 * freq.NATOMS - 6):
                log.Writeonlyfile(
                    '{0:24.4f} {1:9.4f} {2:9.4f}'.format(freq.FREQS[mode], freq.REDMASS[mode], freq.FORCECONST[mode]))
        ...
        for mode, wn in enumerate(freq.FREQS):
            ...
            if freq.FREQS[mode] < 0.0 and val is None and num is None:
            ...
            for atom in range(0, freq.NATOMS):
                for coord in range(0, 3):
                    freq.CARTESIANS[atom][coord] = freq.CARTESIANS[atom][coord] + freq.NORMALMODE[mode][atom][coord] * \
                                                   shift[mode]
        ...
        new_input.Writeonlyfile('%chk=' + file.split(".")[0] + ".chk")
        new_input.Writeonlyfile('%nproc=' + str(nproc) + '\n%mem=' + mem + '\n#p ' + route + '\n\n' + file.split(".")[
            0] + '_' + suffix + '\n\n' + str(freq.CHARGE) + " " + str(freq.MULT))
        for atom in range(0, freq.NATOMS):
            new_input.Writeonlyfile(
                '{0:>2} {1:12.8f} {2:12.8f} {3:12.8f}'.format(freq.ATOMTYPES[atom], freq.CARTESIANS[atom][0],
                                                              freq.CARTESIANS[atom][1], freq.CARTESIANS[atom][2]))
        new_input.Writeonlyfile("\n")
...
def main():
    ...
    parser.add_option("--amp", dest="amplitude", action="store", help="amplitude (default 0.2)", default="0.2",
                      type="float", metavar="AMPLITUDE")
    parser.add_option("--nproc", dest="nproc", action="store", help="number of processors (default 1)", default="1",
                      type="int", metavar="NPROC")
    parser.add_option("--mem", dest="mem", action="store", help="memory (default 4GB)", default="4GB", type="string",
                      metavar="MEM")
    parser.add_option("--route", dest="route", action="store",
                      help="calculation route (defaults to same as original file)", default=None, type="string",
                      metavar="ROUTE")
    parser.add_option("-v", dest="verbose", action="store_true", help="verbose output", default=True, metavar="VERBOSE")
    parser.add_option("--auto", dest="auto", action="store_true", help="turn on automatic batch processing",
                      default=False, metavar="AUTO")
    parser.add_option("--name", dest="suffix", action="store", help="append to file name (defaults to QRC)",
                      default="QRC", type="string", metavar="SUFFIX")
    parser.add_option("-f", "--freq", dest="freq", action="store",
                      help="request motion along a particular frequency (cm-1)", default=None, type="float",
                      metavar="FREQ")
    parser.add_option("--freqnum", dest="freqnum", action="store",
                      help="request motion along a particular frequency (number)", default=None, type="int",
                      metavar="FREQNUM")
    ...
    for file in files:
        ...
        if freq.IM_FREQS == 0 and options.auto != False:
            ...
            if options.freq is None and options.freqnum is None:
                print('o   {} has {} imaginary frequencies: processing'.format(file, freq.IM_FREQS))
            elif options.freq is not None:
                print('o   {} will be distorted along the frequency of {} cm-1: processing'.format(file, options.freq))
            elif options.freqnum is not None:
                print(
                    'o   {} will be distorted along the frequency number {}: processing'.format(file, options.freqnum))
            qrc = gen_qrc(file, options.amplitude, options.nproc, options.mem, options.route, options.verbose,
